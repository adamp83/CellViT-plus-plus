...
This repo is forked to get it running on a HPC which has poor support for Ray. Changes from the original:

- Ray removed

## HPC setup

Build locally on another linux machine and push to docker hub:

```
cd CellViT-plus-plus
docker build -t adampennycuick/cellvit_plusplus:latest .
```
...
docker run --rm --gpus all \
  -v /home/adampennycuick/tmp/cellvit-test-input.ndpi:/data/slide.ndpi \
  -v /home/adampennycuick/models/CellViT-SAM-H-x40-AMP.pth:/data/model.pth \
  -v /home/adampennycuick/tmp/cellvit-test-output:/output \
  adampennycuick/cellvit_plusplus:latest \
  python /app/cellvit/detect_cells.py \
...
- The bounding box for image can be encoded in its name. For example, in the filename _wsi_01_R_2_C_00_cell_9935_bb_118_120_139_137_bb2_451_365_472_382_image.png_, the portion _bb*18_120_139_137*_ represents the coordinates for the bounding box (upper-left xy, lower-right xy) of the cells images and _bb2*451_365_472_382*_ the coordinates of the cell image in the contex image. For the images a WSI is first devided in regions, and each from context images are cropped from the regions. Therefore the name contains the region id R, context image id C and the cells name (image name) _wsi_01_R_2_C_00_cell_9935_ .
</details>
